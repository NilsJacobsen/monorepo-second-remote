...
export type FSInfoResult = 
  | { status: nfsstat3.ERR_STALE |
    ...
    nfsstat3.ERR_SERVERFAULT; stats?: never; rtmax?: never; rtpref?: never; rtmult?: never; wtmax?: never; wtpref?: never; wtmult?: never; dtpref?: never; maxfilesize?: never; timeDelta?: never; properties?: never }
  | { status: number; stats: fs.Stats & {fileId: bigint }; rtmax: number; rtpref: number; rtmult: number; wtmax: number; wtpref: number; wtmult: number; dtpref: number; maxfilesize: bigint; timeDelta: { seconds: number; nseconds: number }; properties: number };

export type FSInfoHandler = (
  handle: Buffer
) => Promise<FSInfoResult>;
...
export async function fsinfo(
  ...
  try {
    ...
    offset += 4;
    infoBlockBuf.writeUInt32BE(Number(maxFileSize & BigInt(0xffffffff)), offset);
    offset += 4;
