...
description: Complete reference for sync in Legit FS—how to sync branches with remotes, push and pull changes, and collaborate across devices.
---
...
<Callout type="error">
  🧪 **Experimental** - Expect frequent changes. 
    ...
    If you need an API key and would like to try sync with GitHub, please contact us via Discord.
</Callout>
...
{/* TODO: ist das wirklich so, dass es von Remote geladen wird, wenn es die nicht gibt locally und kann ich damit auch neue Branches erstellen, wenn es die weder local noch remote gibt? ähnliche Frage auch in der Branching.md*/}
